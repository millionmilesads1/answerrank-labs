...
const SERVICES = [
  ...
  { id: 'revenue', n: '03', badge: 'Growth Service', icon: ChartIcon, title: 'Organic Revenue Capture', tagline: 'Convert AI authority into revenue', desc: "AI search traffic converts at 3–5× the rate of traditional organic search because intent is pre-validated by the AI's recommendation. We build the conversion infrastructure that captures this high-intent traffic.", deliverables: ['AI Traffic Analytics & Attribution', 'Conversion Rate Optimisation', 'High-Intent Landing Page Architecture', 'Revenue Funnel Design & Optimisation', 'Monthly Performance Reporting', 'Continuous Conversion Testing'], timeline: 'Ongoing optimisation', result: '3–5× higher conversion rates from AI-referred traffic' },
]
  ...
  return (
    <div className="relative min-h-screen" style={{ background: '#060608' }}>
      <div aria-hidden="true" className="fixed inset-0 overflow-hidden z-0 pointer-events-none">
        <div className="orb" style={{ width: 600, height: 600, top: '-10%', right: '5%', background: 'radial-gradient(circle, rgba(59,130,246,0.07) 0%, transparent 65%)' }} />
      </div>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-16 items-start">
            <div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link href="/contact" className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl font-semibold text-sm" style={{ background: 'rgba(96,165,250,0.1)', border: '1px solid rgba(96,165,250,0.2)', color: '#93c5fd' }}>Get Free Audit <ArrowRightIcon className="w-4 h-4" /></Link>
            ...
            <div className="hidden lg:flex flex-col gap-3 pt-16">
              {[{ l: 'Foundation', s: 'GEO — 30-day deploy' }, { l: 'Authority', s: 'Citation — 60-day deploy' }, { l: 'Growth', s: 'Revenue — Ongoing' }].map(c => (
                <div key={c.l} className="stat-chip"><div className="text-sm font-bold mb-0.5" style={{ color: '#60a5fa' }}>{c.l}</div><div className="text-xs" style={{ color: T.t }}>{c.s}</div></div>
              ))}
            </div>
          </div>
        </section>
                  ...
                  <div key={s.id} id={s.id} className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center" style={rev ? { direction: 'rtl' } : {}}>
                    <div style={rev ? { direction: 'ltr' } : {}}>
                      <div className="flex items-center gap-3 mb-4">
                        <div className="flex items-center justify-center w-10 h-10 rounded-xl" style={{ background: 'rgba(96,165,250,0.1)', border: '1px solid rgba(96,165,250,0.18)' }}><Icon className="w-5 h-5" style={{ color: '#60a5fa' }} /></div>
                        <span className="eyebrow">{s.badge}</span>
                        <span className="tabular text-xs font-bold" style={{ color: 'rgba(255,255,255,0.12)' }}>{s.n}</span>
                      </div>
                      <h2 className="display-heading mb-2" style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)' }}>{s.title}</h2>
                      <p className="text-base font-semibold mb-4" style={{ color: '#60a5fa' }}>{s.tagline}</p>
                      <p className="text-base leading-relaxed max-w-[54ch] mb-6" style={{ color: T.s }}>{s.desc}</p>
                      <div className="flex items-center gap-3 p-4 rounded-xl mb-6" style={{ background: 'rgba(96,165,250,0.07)', border: '1px solid rgba(96,165,250,0.15)' }}>
                        <TargetIcon className="w-5 h-5 shrink-0" style={{ color: '#60a5fa' }} />
                        <p className="text-sm font-medium" style={{ color: '#93c5fd' }}>{s.result}</p>
                      </div>
                      <div className="flex gap-3 items-center">
                        <div className="eyebrow">{s.timeline}</div>
                        <Link href="/contact" className="group inline-flex items-center gap-1.5 text-sm font-semibold" style={{ color: '#60a5fa' }}>
                          Get Started <span className="w-5 h-5 rounded-full bg-blue-500/10 flex items-center justify-center group-hover:translate-x-0.5 transition-transform duration-300" style={{ transitionTimingFunction: 'cubic-bezier(0.32,0.72,0,1)' }}><ArrowRightIcon className="w-3 h-3" /></span>
                        </Link>
                      </div>
                    </div>
                    <div style={rev ? { direction: 'ltr' } : {}}>
                      <SpotlightCard className="p-7">
                        <h3 className="text-xs font-bold tracking-widest uppercase mb-5" style={{ color: '#60a5fa' }}>What's Included</h3>
                        <ul className="flex flex-col gap-3.5">
                          {s.deliverables.map(d => (
                            <li key={d} className="flex items-start gap-3">
                              <div className="flex items-center justify-center w-5 h-5 rounded-full shrink-0 mt-0.5" style={{ background: 'rgba(96,165,250,0.1)', border: '1px solid rgba(96,165,250,0.18)' }}><CheckIcon className="w-3 h-3" style={{ color: '#60a5fa' }} /></div>
            ...
            <div className="max-w-3xl mx-auto">
              ...
              <div className="doppelrand">
                <div className="doppelrand-core overflow-hidden">
                  <div className="grid grid-cols-3 px-6 py-3.5" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)' }}>
                    ...
                    <div className="text-center text-[10px] font-bold tracking-widest uppercase" style={{ color: '#60a5fa' }}>GEO</div>
                  </div>
                  {COMPARISON.map((row, i) => (
                    <div key={row.aspect} className="grid grid-cols-3 px-6 py-3.5 items-center" style={{ borderBottom: i < COMPARISON.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none', background: i % 2 !== 0 ? 'rgba(255,255,255,0.015)' : 'transparent' }}>
                      <span className="text-sm" style={{ color: T.s }}>{row.aspect}</span>
                      <div className="flex justify-center">
                        {row.traditional ? <CheckIcon className="w-4 h-4" style={{ color: 'rgba(235,235,242,0.22)' }} /> : <span className="w-4 h-px" style={{ background: 'rgba(255,255,255,0.1)' }} />}
                      </div>
                      <div className="flex justify-center">
                        {row.geo ? <CheckIcon className="w-4 h-4" style={{ color: '#60a5fa' }} /> : <span className="w-4 h-px" style={{ background: 'rgba(255,255,255,0.1)' }} />}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-col gap-4">
                  {[{ icon: GlobeIcon, text: 'Coverage across ChatGPT, Gemini, Perplexity & Bing Copilot' }, { icon: LightningIcon, text: '24-hour audit delivery guarantee' }, { icon: TargetIcon, text: 'Personalised strategy for your specific industry' }].map(({ icon: Icon, text }) => (
                    <div key={text} className="flex items-center gap-3">
                      <div className="flex items-center justify-center w-9 h-9 rounded-xl shrink-0" style={{ background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.18)' }}><Icon className="w-4 h-4" style={{ color: '#60a5fa' }} /></div>
