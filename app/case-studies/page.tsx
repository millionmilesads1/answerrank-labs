  ...
  return (
    <div className="relative min-h-screen" style={{ background: '#ffffff' }}>
      <div aria-hidden="true" className="fixed inset-0 overflow-hidden z-0 pointer-events-none">
        <div className="orb" style={{ width: 700, height: 600, top: '0%', left: '0%', background: 'radial-gradient(circle, rgba(29,78,216,0.04) 0%, transparent 65%)' }} />
      </div>
            ...
            <div className="hidden lg:grid grid-cols-2 gap-3 pt-16">
              {AGG.map(s => (
                <div key={s.v} className="stat-chip">
                  ...
                  <div className="text-xs font-semibold mb-0.5" style={{ color: T.p }}>{s.l}</div>
                  <div className="text-[10px]" style={{ color: T.t }}>{s.s}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              {CASES.map(cs => {
                ...
                return (
                  <SpotlightCard key={cs.id}>
                    <div className="px-8 py-5 flex items-center justify-between" style={{ background: cs.featured ? 'rgba(29,78,216,0.04)' : '#f8fafc', borderBottom: '1px solid rgba(0,0,0,0.07)', borderRadius: '1.5rem 1.5rem 0 0' }}>
                      <div className="flex items-center gap-3">
                        <div className="flex items-center justify-center w-9 h-9 rounded-xl" style={{ background: 'rgba(29,78,216,0.07)', border: '1px solid rgba(29,78,216,0.15)' }}><Icon className="w-4 h-4" style={{ color: '#1d4ed8' }} /></div>
                        <div>
                          <div className="text-[10px] font-bold tracking-widest uppercase" style={{ color: '#1d4ed8' }}>{cs.cat}</div>
                          <div className="text-xs mt-0.5" style={{ color: T.t }}>{cs.co}</div>
                        </div>
                      </div>
                    ...
                    <div className="p-8">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-8">
                        <div className="flex flex-col gap-5">
                          ...
                          <div>
                            <h3 className="text-[10px] font-bold tracking-widest uppercase mb-2.5" style={{ color: '#1d4ed8' }}>Our Solution</h3>
                            <ul className="flex flex-col gap-2">
                              {cs.solution.map(s => <li key={s} className="flex items-start gap-2.5"><CheckIcon className="w-4 h-4 shrink-0 mt-0.5" style={{ color: '#1d4ed8' }} /><span className="text-sm" style={{ color: T.s }}>{s}</span></li>)}
                            </ul>
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {cs.results.map(r => (
                              <div key={r.l} className="p-4 rounded-xl" style={{ background: '#f8fafc', border: '1px solid rgba(0,0,0,0.07)' }}>
                                <div className="tabular text-2xl font-black leading-none mb-1" style={{ color: '#1d4ed8', letterSpacing: '-0.05em' }}>{r.m}</div>
                                <div className="text-xs leading-snug" style={{ color: T.t }}>{r.l}</div>
                              </div>
                            ))}
                          </div>
                        </div>
                      </div>
                      <div className="rounded-xl p-5" style={{ background: 'rgba(29,78,216,0.04)', border: '1px solid rgba(29,78,216,0.1)' }}>
          ...
          <section className="py-24 px-6 sec-divider">
            <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14 items-start">
              <div>
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl" style={{ background: 'rgba(22,163,74,0.05)', border: '1px solid rgba(22,163,74,0.14)' }}>
                  <CheckIcon className="w-5 h-5 shrink-0" style={{ color: '#16a34a' }} />
                  <div>
                    <p className="text-sm font-bold" style={{ color: T.p }}>98% client retention rate</p>
                    <p className="text-xs mt-0.5" style={{ color: T.t }}>GEO results compound — clients stay because authority grows</p>
                  </div>
                </div>
              </div>
