...
const PHASES = [
  ...
  { n: '05', week: 'Ongoing', icon: ChartIcon, title: 'Monitor, Compound & Expand', desc: "AI search authority compounds over time. We continuously monitor your citation footprint, track competitor movements, and expand into adjacent categories. Monthly reporting with precision metrics.", deliverables: ['Weekly Citation Monitoring', 'Share of AI Voice Tracking', 'Competitor Displacement Analysis', 'Revenue Attribution Dashboard'] },
]
  ...
  return (
    <div className="relative min-h-screen" style={{ background: '#ffffff' }}>
      <div aria-hidden="true" className="fixed inset-0 overflow-hidden z-0 pointer-events-none">
        <div className="orb" style={{ width: 600, height: 600, top: '-5%', left: '20%', background: 'radial-gradient(circle, rgba(29,78,216,0.04) 0%, transparent 65%)' }} />
      </div>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-16 items-start">
              ...
              <div className="flex items-center gap-0 flex-wrap gap-y-2">
                {['Wk 1', 'Wk 2', 'Wks 3–5', 'Wks 6–10', 'Ongoing'].map((w, i) => (
                  <div key={w} className="flex items-center">
                    {i > 0 && <div className="w-5 h-px mx-1.5" style={{ background: 'rgba(0,0,0,0.12)' }} />}
            ...
            <div className="hidden lg:flex flex-col gap-3 pt-16">
              {[{ v: '30–45', l: 'Days to first citations' }, { v: '90', l: 'Days to category authority' }, { v: '24h', l: 'Audit delivery' }].map(c => (
                <div key={c.l} className="stat-chip"><div className="tabular text-xl font-black leading-none mb-1" style={{ color: '#1d4ed8', letterSpacing: '-0.05em' }}>{c.v}</div><div className="text-xs" style={{ color: T.t }}>{c.l}</div></div>
              ))}
            </div>
          </div>
        </section>
          ...
          <section className="py-12 px-6 sec-divider" style={{ background: '#f8fafc' }}>
            <div className="max-w-4xl mx-auto">
              {PHASES.map((phase, idx) => {
                ...
                return (
                  <div key={phase.n} className="relative flex gap-8 py-10" style={{ borderTop: idx > 0 ? '1px solid rgba(0,0,0,0.07)' : 'none' }}>
                    <div className="flex flex-col items-center shrink-0 w-12">
                      <div className="flex items-center justify-center w-12 h-12 rounded-2xl" style={{ background: 'rgba(29,78,216,0.07)', border: '1px solid rgba(29,78,216,0.16)' }}>
                        <Icon className="w-5 h-5" style={{ color: '#1d4ed8' }} />
                      </div>
                      {idx < PHASES.length - 1 && <div className="w-px mt-4" style={{ background: 'rgba(29,78,216,0.15)', height: 40 }} />}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="eyebrow">{phase.week}</span>
                        <span className="tabular text-xs font-bold" style={{ color: 'rgba(0,0,0,0.14)' }}>{phase.n}</span>
                      </div>
                      ...
                      <div className="rounded-xl p-4" style={{ background: 'rgba(29,78,216,0.04)', border: '1px solid rgba(29,78,216,0.1)' }}>
                        <h4 className="text-[10px] font-bold tracking-widest uppercase mb-3" style={{ color: '#1d4ed8' }}>Deliverables</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {phase.deliverables.map(d => (
                            <div key={d} className="flex items-start gap-2">
                              <div className="w-1 h-1 rounded-full shrink-0 mt-1.5" style={{ background: '#1d4ed8' }} />
        ...
        <RevealSection>
          <section className="py-20 px-6 sec-divider">
            <div className="max-w-5xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-px rounded-2xl overflow-hidden" style={{ background: 'rgba(0,0,0,0.07)', border: '1px solid rgba(0,0,0,0.08)' }}>
                {[
                  { period: 'Days 1–30', title: 'Foundation & Audit', items: ['Complete AI Search Audit', 'Authority Blueprint Delivered', 'Content Architecture Begins'] },
                  { period: 'Days 30–60', title: 'Build & Deploy', items: ['20+ AI-Native Content Assets Live', 'Structured Data Deployed', 'Citation Outreach Begins'], featured: true },
                  { period: 'Days 60–90', title: 'Authority & Results', items: ['First AI Citation Improvements', 'Share of AI Voice Tracking Live', 'Revenue Attribution Active'] },
                ].map(t => (
                  <div key={t.period} className="p-7" style={{ background: t.featured ? 'rgba(29,78,216,0.05)' : '#ffffff' }}>
                    <span className="eyebrow mb-4 inline-block" style={t.featured ? { background: 'rgba(29,78,216,0.1)', border: '1px solid rgba(29,78,216,0.22)' } : {}}>{t.period}</span>
                    <h3 className="text-base font-bold mb-4" style={{ color: T.p }}>{t.title}</h3>
                    <ul className="flex flex-col gap-2">
                      {t.items.map(item => (
                        <li key={item} className="flex items-start gap-2">
                          <CheckIcon className="w-4 h-4 shrink-0 mt-0.5" style={{ color: t.featured ? '#1d4ed8' : 'rgba(10,10,15,0.28)' }} />
          ...
          <section className="py-24 px-6 sec-divider">
            <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14 items-start">
              <div>
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl" style={{ background: 'rgba(29,78,216,0.05)', border: '1px solid rgba(29,78,216,0.12)' }}>
                  <ClockIcon className="w-5 h-5 shrink-0" style={{ color: '#1d4ed8' }} />
                  <div>
                    <p className="text-sm font-bold" style={{ color: T.p }}>Audit delivered in 24 hours</p>
                    <p className="text-xs mt-0.5" style={{ color: T.t }}>Response to your enquiry within 2 hours</p>
                  </div>
                </div>
              </div>
