  ...
  return (
    <div className="relative min-h-screen" style={{ background: '#060608' }}>
      <div aria-hidden="true" className="fixed inset-0 overflow-hidden z-0 pointer-events-none">
        <div className="orb" style={{ width: 600, height: 600, top: '-5%', right: '5%', background: 'radial-gradient(circle, rgba(59,130,246,0.06) 0%, transparent 65%)' }} />
      </div>
      <div className="relative z-10">
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-16 items-start">
            ...
            <div className="hidden lg:grid grid-cols-2 gap-3 pt-16">
              {[{ v: '2023', l: 'Founded' }, { v: '50+', l: 'Clients' }, { v: '98%', l: 'Retention' }, { v: '3', l: 'AI Platforms' }].map(s => (
                <div key={s.l} className="stat-chip"><div className="tabular text-xl font-black leading-none mb-1" style={{ color: '#60a5fa', letterSpacing: '-0.05em' }}>{s.v}</div><div className="text-xs" style={{ color: T.t }}>{s.l}</div></div>
              ))}
            </div>
          </div>
        </section>
        ...
        <RevealSection>
          <section className="py-24 px-6 sec-divider">
            <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
              <div>
                <div className="eyebrow w-fit mb-5" style={{ background: 'rgba(96,165,250,0.07)', border: '1px solid rgba(96,165,250,0.15)', color: '#60a5fa' }}><SparkleIcon className="w-3 h-3" />Why We Built This</div>
              ...
              <div className="p-7 rounded-2xl" style={{ background: 'rgba(96,165,250,0.05)', border: '1px solid rgba(96,165,250,0.12)' }}>
                ...
                <div className="flex flex-col gap-3.5">
                  {['67% of enterprise buyers now use AI assistants for vendor research', 'AI search queries convert at 3–5× the rate of traditional search', 'Brands establishing AI authority today will own their categories for years', 'Only 8% of brands currently have any systematic AI search strategy'].map(fact => (
                    <div key={fact} className="flex items-start gap-3"><CheckIcon className="w-4 h-4 shrink-0 mt-0.5" style={{ color: '#60a5fa' }} /><span className="text-sm leading-relaxed" style={{ color: T.s }}>{fact}</span></div>
                  ))}
                </div>
              </div>
            </div>
                ...
                {VALUES.map(v => {
                  ...
                  return (
                    <SpotlightCard key={v.t} className="p-6">
                      <div className="flex items-center justify-center w-10 h-10 rounded-xl mb-4" style={{ background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.18)' }}>
        ...
        <RevealSection>
          <section className="py-24 px-6 sec-divider">
            <div className="max-w-5xl mx-auto">
              <div className="mb-10"><div className="eyebrow w-fit mb-4"><SparkleIcon className="w-3 h-3" />Our Edge</div><h2 className="display-heading" style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)' }}>What Separates Answer Rank Labs</h2></div>
              {DIFF.map((d, i) => (
                <div key={d.n} className="feature-row">
                  <div className="grid grid-cols-1 md:grid-cols-[60px_220px_1fr] gap-6 md:gap-10 items-start">
                    <span className="hidden md:block tabular font-black font-mono leading-none text-4xl" style={{ color: 'rgba(255,255,255,0.06)' }}>{d.n}</span>
        ...
        <RevealSection>
          <section className="py-20 px-6 sec-divider" style={{ background: 'rgba(255,255,255,0.015)' }}>
            <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
              {[{ flag: '🇺🇸', m: 'United States', d: "Deep expertise in US enterprise technology, SaaS, professional services, and B2B sectors. We understand how US enterprise buyers use AI assistants in their purchase process." }, { flag: '🇮🇳', m: 'India', d: "India's digital transformation is accelerating AI search adoption faster than any other market. We have native expertise in the sectors driving India's enterprise growth." }].map(m => (
              ...
              <div>
                ...
                <div className="flex flex-col gap-3">
                  {['US & India enterprise brands', 'Response within 2 business hours', 'Free audit with no obligation'].map(item => (
                    <div key={item} className="flex items-center gap-2.5"><CheckIcon className="w-4 h-4 shrink-0" style={{ color: '#60a5fa' }} /><span className="text-sm" style={{ color: T.s }}>{item}</span></div>
                  ))}
                </div>
              </div>
