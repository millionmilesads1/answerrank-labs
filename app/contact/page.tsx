...
const INCLUDES = [
  ...
  { icon: GlobeIcon, t: 'Category Gap Analysis', d: 'The specific content and citation gaps preventing your brand from appearing in AI answers for your most valuable category queries.' },
  ...
  return (
    <div className="relative min-h-screen" style={{ background: '#060608' }}>
      <div aria-hidden="true" className="fixed inset-0 overflow-hidden z-0 pointer-events-none">
        <div className="orb" style={{ width: 700, height: 700, top: '-10%', left: '20%', background: 'radial-gradient(circle, rgba(59,130,246,0.08) 0%, transparent 65%)' }} />
      </div>
      <div className="relative z-10">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2">
                {['50+ brands served', '98% retention rate', 'US and India', 'Results in 30 days'].map(t => (
                  <div key={t} className="flex items-center gap-2 text-sm" style={{ color: T.t }}><CheckIcon className="w-4 h-4 shrink-0" style={{ color: '#4ade80' }} />{t}</div>
                ))}
              </div>
            </div>
            <div className="hidden lg:flex flex-col gap-2.5 pt-16">
              {STEPS.slice(0, 3).map(ps => (
                <div key={ps.n} className="stat-chip">
                  <div className="tabular text-xs font-bold mb-0.5" style={{ color: '#60a5fa' }}>{ps.n}</div>
        ...
        {/* FORM + SIDEBAR */}
        <RevealSection>
          <section className="py-12 px-6 sec-divider" style={{ background: 'rgba(255,255,255,0.015)' }}>
            <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-10">
              <div className="lg:col-span-3">
                ...
                <div className="mt-4 text-center">
                  ...
                  <a href="https://wa.me/918796394750" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold" style={{ background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.18)', color: '#4ade80' }}>
                      ...
                      {INCLUDES.map(item => {
                        ...
                        return (
                          <div key={item.t} className="flex gap-3">
                            <div className="flex items-center justify-center w-7 h-7 rounded-lg shrink-0" style={{ background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.18)' }}><Icon className="w-3.5 h-3.5" style={{ color: '#60a5fa' }} /></div>
                            <div><p className="text-sm font-semibold" style={{ color: T.p }}>{item.t}</p><p className="text-xs leading-relaxed mt-0.5" style={{ color: T.t }}>{item.d}</p></div>
                          </div>
                        )
                      })}
                    </div>
                    ...
                    <div className="flex flex-col gap-3.5">
                      {STEPS.map(ps => (
                        <div key={ps.n} className="flex gap-3">
                          <div className="flex items-center justify-center w-5 h-5 rounded-lg shrink-0 text-[10px] font-bold tabular" style={{ background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.18)', color: '#60a5fa' }}>{ps.n}</div>
                          <div><p className="text-sm font-semibold" style={{ color: T.p }}>{ps.t}</p><p className="text-xs mt-0.5" style={{ color: T.t }}>{ps.d}</p></div>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <section className="py-24 px-6 sec-divider">
            <div className="max-w-4xl mx-auto">
              <h2 className="display-heading mb-4" style={{ fontSize: 'clamp(1.8rem, 3.5vw, 3.2rem)' }}>
                The Window Is Open. <span style={{ color: 'rgba(235,235,242,0.2)' }}>But Not Forever.</span>
              </h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-10">
                {[{ s: '8%', l: 'of brands have an AI search strategy', c: '92% gap = your opportunity' }, { s: '67%', l: 'of enterprise buyers use AI for vendor research', c: 'and the number is growing' }, { s: '3–5×', l: 'higher conversion from AI-referred traffic', c: 'vs. traditional organic' }].map(item => (
                  <div key={item.s} className="stat-chip">
                    <div className="tabular text-2xl font-black leading-none mb-1" style={{ color: '#60a5fa', letterSpacing: '-0.05em' }}>{item.s}</div>
